...
/***/function (module, exports, __webpack_require__) {
	...
	$(function () {
		...
		$mobMenu.mmenu({
			...
			"navbars": [{
				"position": "bottom",
				"content": ["<button class='link js-popup-trigger js-btn-mob' data-rel='sign-up'>Sign Up</button>", "<button class='link js-popup-trigger js-btn-mob' data-rel='sign-in'>Sign In</button>"]
			}]
		});

		function stickyTop() {
			var $stickyEl = $('.js-sticky-header');
			var $constHeight = 246;
			var $scrollTop = $(window).scrollTop();
			var $tabTrigger = $('.js-tab-trigger');

			if ($scrollTop > 0) {
				$tabTrigger.unbind('mouseenter mouseleave');
			}

			// $stickyEl.css({ 'top': '-200px' });
			if ($scrollTop > $constHeight) {
				$stickyEl.addClass('flag-sticky', 2000, 'easeInOutQuad');
			} else {
				$stickyEl.removeClass('flag-sticky', 2000, 'easeInOutQuad');
			}
		...
		function hideMenu() {
			var $mmenu = $('.mm-menu');
			var $openedMenuClass = 'mm-menu_opened';

			if ($mmenu.hasClass($openedMenuClass)) {
				$mmenu.removeClass($openedMenuClass);
			}
		...
		$(document).on('scroll', stickyTop).on('click', '.js-btn-mob', hideMenu).on('keyup', '.input-select', preventType);
	});
...
/***/function (module, exports) {
   ...
   */
    ...
    */
			...
			_createClass(Loader, [{
				key: 'show',
				value: function show(setTransparent) {
					// If loader didn't display yet, we are displaying it
					if (this._isLoaderStarted()) {
						var lookType = setTransparent ? 'transparent-mode' : '';
     ...
     */
				...
				key: '_isLoaderStarted',
				value: function _isLoaderStarted() {
...
/***/function (module, exports) {
   ...
   */
		...
		$(document).on('click', cssTrigger, onExpandTriggerClick).on('click', '.js-popup-close', closeAllExpands).on('click', '.js-collapse', collapseText);
	})(jQuery);
...
/***/function (module, exports) {
	...
	$(function () {
		...
		$tabTrigger.on('hover', function () {
			var $this = $(this);

			if ($this.find('.tab-target').is(':visible')) {
				$this.find('.tab-target').fadeOut('medium');
			} else {
				$this.find('.tab-target').fadeIn('medium');
			}
...
/***/function (module, exports) {
    ...
    */
			setShown: function setShown(popupRelation) {
				var currentTarget = $('.js-popup-target').filter('[data-rel=' + popupRelation + ']');
   ...
   */
		function hidePopup(e) {
			...
			if ($footer.hasClass('flag-hide')) {
				$footer.removeClass('flag-hide');
			} else {
				return false;
			}
		}

		$(document).on('click.popup', '.js-popup-trigger', showPopup).on('click.popup', '.js-popup-close', hidePopup);
	})(jQuery);
    ...
    */
     ...
     */
				...
				value: function updateWidth() {
					var init_style = this.$block.get(0).style.cssText;
     ...
     */
				...
				key: 'process',
				value: function process(winScroll) {
					if (!this._enabled) {
						return;
					}

					var container_top = this.$parentContainer.offset().top + (parseInt(this.$parentContainer.css('padding-top')) || 0) + this.opts.startTop;
					var blockHeight = this.$block.outerHeight(true);
					var containerHeight = this.$parentContainer.height();
					var scrollFrom = container_top - this.opts.topOffset;
					var scrollTo = scrollFrom + containerHeight - blockHeight - this.opts.bottomOffset;
     ...
     */
				...
				key: 'rared',
				value: function rared(callback, time) {
					var sleeping = void 0,
					    sleeping_call = void 0,
					    that = void 0,
					    args = void 0;
				...
				value: function addDefaultValidateRules() {
					...
					if (window.validate) {
						...
						window.validate.validators.checkbox = function (value, options) {
							if (!!_this5.$form.find(_this5.checkbox).length) {
								var message = options.message || _this5.messages.oneCheckbox;

								return _this5.$form.find(_this5.checkbox + ':checked').length < 1 ? message : null;
							}
						};

						// Group checkbox
						window.validate.validators.checkboxGroup = function (value, options) {
							if (!!_this5.$form.find(_this5.checkboxGroup).length) {
								var message = options.message || _this5.messages.groupCheckbox;
								var minChecked = options.minChecked || 1;

								return _this5.$form.find(_this5.checkboxGroup + ' input[type="checkbox"]:checked').length < minChecked ? message : null;
							}
						};

						// File
						window.validate.validators.file = function (value, options) {
							var $file = _this5.$form.find(_this5.file);

							if (!!$file.length) {
								var message = options.message || _this5.messages.fileValidate;
								var maxFileSize = options.maxFileSize || _this5.maxFileSize;
								var required = options.required || false;

								var file = $file.get(0).files[0];
								var isValid = file && file.size < maxFileSize ? null : message;

								return required ? file ? isValid : _this5.messages.fileRequired : file ? isValid : null;
							}
						};
					}
				}
     ...
     */
				...
				value: function renderErrors(error) {
					...
					$.each(error, function (i, val) {
						var $error = _this6.$form.find('[data-error="' + i + '"]');
						var $field = _this6.$form.find('[name="' + i + '"]');
...
/***/function (module, exports, __webpack_require__) {
  ...
  */
		...
		}).call(this, true ? /* istanbul ignore next */exports : null, true ? /* istanbul ignore next */module : null, __webpack_require__(18));
...
/***/function (module, exports) {
	...
	$(function () {
		...
		$('.sliders').each(function () {
			var $pagPrev = $('.pag_controls .arrow-prev', this);
			var $pagNext = $('.pag_controls .arrow-next', this);
			var $projectsItem = $('.slide li', this);
			var $pagBlock = $('.pag_controls', this);
			var $pagItem = $('.pagination li', this);

			$pagNext.click(function () {
				var index = $pagItem.filter('.active').index();
				if (index == $pagItem.length - 1) {
					index = -1;
				}
				$pagItem.eq(index + 1).addClass('active').siblings().removeClass('active');
				var index = $projectsItem.filter('.active').index();
				if (index == $projectsItem.length - 1) {
					index = -1;
				}
				$projectsItem.eq(index + 1).addClass('active').siblings().removeClass('active');
			});
			...
			$pagItem.click(function () {
				$projectsItem.eq($(this).index()).addClass('active').siblings().removeClass('active');
...
/***/function (module, exports) {
   ...
   */
		...
		var Subscribe = function (_FormValidation2) {
			_inherits(Subscribe, _FormValidation2);
     ...
     */
				value: function init() {
					...
					_get(Subscribe.prototype.__proto__ || Object.getPrototypeOf(Subscribe.prototype), 'init', this).call(this);
     ...
     */
				...
				key: 'clearErrors',
				value: function clearErrors() {
					_get(Subscribe.prototype.__proto__ || Object.getPrototypeOf(Subscribe.prototype), 'clearErrors', this).call(this);
					this.$form.find('.js-error-form').html('');
				}
...
/***/function (module, exports) {
   ...
   */
		...
		var FormLogin = function (_FormValidation3) {
			_inherits(FormLogin, _FormValidation3);

			/**
    * Constructor (init events)
    */
			function FormLogin(form, options) {
				_classCallCheck(this, FormLogin);

				// Validate Options
				var _this9 = _possibleConstructorReturn(this, (FormLogin.__proto__ || Object.getPrototypeOf(FormLogin)).call(this, form, options));

				_this9.optionsValidate = {
					fullMessages: false
				};

				_this9.$form.find(_this9.inputs).on('focus', function () {
					return _this9.clearErrors();
				});
				return _this9;
			}
    ...
    */
			...
			_createClass(FormLogin, [{
				...
				value: function getConstraints() {
					return {
						'signin-username': {
							presence: {
								message: 'Please type your name or e-mail'
							}
						},
     ...
     */
				...
				value: function _sendAjax(data) {
					...
					alert('success!');
					this.$form.closest('.js-popup-target').fadeOut(100);
				}
...
/***/function (module, exports) {
   ...
   */
		...
		var FormLogin = function (_FormValidation4) {
			_inherits(FormLogin, _FormValidation4);
    ...
    */
			function FormLogin(form, options) {
				...
				// Validate Options
				var _this10 = _possibleConstructorReturn(this, (FormLogin.__proto__ || Object.getPrototypeOf(FormLogin)).call(this, form, options));

				_this10.optionsValidate = {
					fullMessages: false
				};

				_this10.$form.find(_this10.inputs).on('focus', function () {
					return _this10.clearErrors();
				});
				return _this10;
			}
