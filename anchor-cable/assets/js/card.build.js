...
/***/function (module, exports, __webpack_require__) {
	...
	;(function ($) {
		...
		$(document).on('blur', '.js-input-tel', function (e) {
			var $inputTel = $(e.currentTarget);

			if ($inputTel.length !== 0) {
				$inputTel.closest('.input-field').addClass('is-dirty');
			}
		});
	})(jQuery);
...
/***/function (module, exports) {
   ...
   */
		...
		var SendBooking = function (_FormValidation) {
			_inherits(SendBooking, _FormValidation);

			function SendBooking() {
				_classCallCheck(this, SendBooking);

				return _possibleConstructorReturn(this, (SendBooking.__proto__ || Object.getPrototypeOf(SendBooking)).apply(this, arguments));
			}

			_createClass(SendBooking, [{
				key: 'init',
     ...
     */
				value: function init() {
					...
					_get(SendBooking.prototype.__proto__ || Object.getPrototypeOf(SendBooking.prototype), 'init', this).call(this);
     ...
     */
				...
				key: 'clearErrors',
				value: function clearErrors() {
					_get(SendBooking.prototype.__proto__ || Object.getPrototypeOf(SendBooking.prototype), 'clearErrors', this).call(this);
     ...
     */
				...
				value: function submit() {
					...
					var qtyItems = $('.js-input-qty').val();
					var total = parseInt($('.js-price').html().toString());
					var radioType = $('.js-radio-type');
					var radioItem = radioType.find(this.inputs).filter(function () {
						return $(this).prop('checked');
					});
					var promoCode = $('.js-input-promo').val();
					...
					formData.push({
						...
						'name': 'promo',
						'value': promoCode.length !== '' ? promoCode : ''
					});
     ...
     */
		...
		$(function () {
			new SendBooking($('.js-form-booking'));
		});
	})(jQuery);
...
/***/function (module, exports) {
	...
	;(function ($) {
		...
		var $price = 2480,
		    $qty = parseInt($('.js-input-qty').val()) || 1;

		(function () {
			var $origin = Math.round($qty * $price);

			return $('.js-price').html($origin);
		})();

		// Booking of items
		function calculateQty() {
			var $finalPrice = $('.js-price').html($price);
			var $qtyChanges = parseInt($('.js-input-qty').val());
			var $promo = $('.js-input-promo');
			var $result = 0;

			$result = Math.round($qtyChanges * $price);
			$promo.val('');
			$promo.closest('.input-field').removeClass('is-discounted');
		...
		function getDiscount(e) {
			...
			$result = Math.round($qtyChanges * $newPrice);
